...
// POP UP //
document.addEventListener('DOMContentLoaded', () => {
    const popupOverlay = document.querySelector('.popup-menu-overlay');
    const popupClose = document.getElementById('btn-close-menu-popup');
    const checkoutButton = document.getElementById('open-menu');

    if (checkoutButton && popupOverlay && popupClose) {
        checkoutButton.addEventListener('click', () => {
            popupOverlay.style.display = 'block'; // Показываем поп-ап
        });

        popupClose.addEventListener('click', () => {
            popupOverlay.style.display = 'none'; // Скрываем поп-ап
        });
    }
});
